...
if [ $DISPLAY ]; then
	if ( ! xprop -root | grep KDE > /dev/null ) && [[ -z $IGNORE_NOKDE ]]; then
		...
		function func_getcurrentwindow()
		{
			...
			eval xprop $getspec | grep ^"_NET_ACTIVE_WINDOW" | grep --only-matching "0x[0-9a-fA-F]*" --color=never			
		}
		
		if [[ $DO_UNBLUR == 1 ]]; then
			xprop -remove _KDE_NET_WM_BLUR_BEHIND_REGION -id `func_getcurrentwindow`
		else
