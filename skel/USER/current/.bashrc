...
# $- contains 'i' aka interactive (required)
[[ $- =~ i ]] || { return; exit; }
...
#{{{ MAIN CONFIGURATION
	declare -xgi BASH_IS_INITIALIZING=1

	[[ ! -v TTY ]] 				&& 			  TTY=$(tty)
	[[ ! -v PARENT_PROCESS ]] 	&& PARENT_PROCESS=$(ps --format='%c' --no-header $PPID)
	...
	declare -xg BASH_CTAGS_FILE=~/.config/bashrc/tags
	declare -xga BASH_CTAGS_INC=(
	~/.config/bashrc/
	~/.bash*!(*.swp)
	/etc/bash.bashrc
	/etc/bash_completion
	...
	declare -xA BASH_CONFIG_DIRS=(
		...
		[session]="${BASH_CONFIG_ROOTS[user]}/session/${BASH_SESSION_ID}"
...
declare -gx HISTCONTROL=ignoreboth
declare -gx HISTSIZE=100000
declare -gx HISTFILESIZE=200000
# }}}
# {{{ DEFAULT PROMPT (PS1, PS2 and PS3 - for main prompt, more output prompt, select menu prompt, respectively)
declare -gx PS1='lv$[SHLVL-2] r$? \[\e[38;5;$[$?+2]m\]\u@\h \[\e[38;5;$[${#PWD}+32]m\]\w\[\e[40;30;1m\] \$\[\e[0m\] '
declare -gx PS2='continued > '                           
declare -gx PS3='selection:' # NOTE, IMPORTANT!!: PS3 does not get evaluated like PS1/PS2 so embedded codes make no difference (or execution of $()s either)
# }}}
# {{{ STARTING DIRECTORY (WHERE BASH INVOKED FROM), FIND A TEMPORARY DIRECTORY LOCATION
...
#}}}
# {{{ ENVIRONMENT-DEPENDANT PROGRAM EXPORTS
...
if [[ $TTY != NONE ]]; then
	...
	alias lb='clear; exec `which bash`; true'
...
# }}}
# {{{ __bash_apply_local_completions
