...
# }}}
#{{{ SPECIAL NOTES
# 1a. To use tags, make sure you compile the tags file with the makebashtags command from this script
# 1b. YOU WILL NEED ctags-exhuberant to compile the tags when things change, especially functions.
...
#{{{ PREINIT
# {{{ FUNCTIONS NEEDED BY ALL CONFIG FILES 
...
function slice_func()
{
	...
	if [[ $# -gt 1 ]]; then
		...
		cat ~/.bashrc | perl -ne "print if /^function $1/ .. /^}/;" > $TMPFILE
	fi
	if [[ ! -s $TMPFILE ]]; then
		WARN "$TMPFILE -- while sourcing $* -- file did not gain strength."
	else
		source "$TMPFILE"
	fi
	rm -f "$TMPFILE"
...
declare -g BASH_SESSION_EXPERIMENTATION_MODES="HOOKPROC" # also, HOOKPROC, ROBUST_INIT, PERSISTENT
...
#{{{ MAIN CONFIGURATION 
	...
	declare -xA BASHCFG=( 
		...
		[flags]=$BCROOT/flags/
		[dev]=$BCROOT/dev/
		[ps1]=$BCROOT/ps1colors/
		[sessionroot]=$BCROOT/setlocalstack/ 
		[ushell]=$BCROOT/shells/
		)
#{{{ GXBASE 
...
#	[[ -r ~/.bash_hash ]] && source ~/.bash_hash
	[[ -r ~/.bash_settings ]] && source ~/.bash_settings
	[[ -r ~/.bash_vars ]] && source ~/.bash_vars
	[[ -r ~/.bash_helpers ]] && source ~/.bash_helpers
...
_cur_row ROW; [[ $ROW -ge $ROWS ]] && tput cuu 2
...
if [[ $TTY != NONE ]]; then
	...
	# this DOES include shell extensions that use bash at the same interaction level (like .bash_git (gitsh))
...
function setup-bashrc()
{
	if ! which dialog &> /dev/null; then
		...
		test -r ~/.bashrc_setup && source ~/.bashrc_setup
...
function eb()
{
	while ! bmenu "Edit Bash Configuration" "rc|Main Configuration File" "aliases|Aliases" "completion|Completion" "dircolors.db|DirectoryColors" "git|GIT Integration" "hash|Hashtable" "helpers|Helpers" "history|History*"	"logout|Logout" "settings|User Settings" "vars|User Variables" "xopen|X-Open All Files" ; do
   ...
   [[ -v __ECHOER_COLORCACHE ]] || declare -gAx __ECHOER_COLORCACHE 
	__isflag $__FLG && {
		if [[ -v __ECHOER_COLORCACHE[$__FLG] ]]; then
			echo "${__ECHOER_COLORCACHE[$__FLG]}"
		else		
...
function makebashtags()
{
	...
	INFO "xtra locations added: ${#EXTRA_LOCS[@]}"
...
# {{{    slice() [manpage] [topic] [indentlevel (default 7)
function slice()
{	
   local SLICE_OUTPUT=$(mktemp)	
	...
	[[ $SLICE_TOPIC ]] && [[ $SLICE_SEARCH ]] && { 
		if man "$SLICE_TOPIC" | perl -ne 'print if /^\s{'$SLICE_INDENT'}'$SLICE_SEARCH'/ .. /^(\s{'$SLICE_INDENT'}(?!'$SLICE_SEARCH'| )|[A-Z]+)/' | head -n-1 > $SLICE_OUTPUT; then
			cat "$SLICE_OUTPUT"
			DETA "Slice Output is $SLICE_OUTPUT, which is `stat $SLICE_OUTPUT --format='%s'`"
		else
			if [[ -s $SLICE_OUTPUT ]]; then
				echo "Returned an error getting the slice, the partial output was returned:"
				cat "$SLICE_OUTPUT"
			else 			
				echo "Returned an error getting the slice, please check your regular expression and try again."
			fi
			rm "$SLICE_OUTPUT"
		fi
	} || {
...
#	[[ -r ~/.bash_completion ]] && source ~/.bash_completion
	...
	echo "* Processing Bash Configuration"
#}}}
...
#trap DEBUG
   ...
   function voperate()
	...
	voperate DCOND CLEANUP_RETURN_TO_GIT,1 DDIR CLEANUP_SAFE_LOCATION SWD ZOLDWD
